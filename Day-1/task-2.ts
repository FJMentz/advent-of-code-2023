...
// Some funnies where the first letter and last letter of a number overlaps
const numbers = [{'oneight':'18'}, {'twone': '21'}, {'threeight': '38'}, {'fiveight':'58'}, {'sevenine': '79'}, {'eightwo': '82'}, {'eighthree': '83'}, {'nineight': '98'},
{'one':'1'},{'two':'2'}, {'three': '3'}, {'four': '4'}, {'five': '5'}, {'six': '6'}, {'seven': '7'}, {'eight': '8'}, {'nine': '9'}];

let answer = 0;

input.forEach(x => {
    let inputString = getNums(x);
    // concat first and last
    let result = calcFirst(inputString) + calcLast(inputString);
...
function calcFirst(input: string):string {
...
function calcLast(input: string):string {
    if (input && input.length > 0) {
...
function getNums(input: string) {
    let result = convertWordToNum(input);
    let numb = result.match(/\d/g);
    return numb?.join('');
}

// Convert numers that are spelled out as numbers
function convertWordToNum(input: string): string {
    let result = input;
